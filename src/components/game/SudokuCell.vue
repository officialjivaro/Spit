...
.sudoku-cell {
  ...
  padding: 0;
  border: 0;
  border-inline-end: 1px solid color-mix(in srgb, var(--sudoku-grid-color) 52%, transparent);
  border-block-end: 1px solid color-mix(in srgb, var(--sudoku-grid-color) 52%, transparent);
  border-radius: 0;
...
.sudoku-cell--wrong {
  background: rgba(255, 190, 203, 0.99);
  color: #8c1435;
}
...
.sudoku-cell--hinted::after {
  position: absolute;
  inset: 0.16rem;
  border: 2px dashed rgba(181, 106, 127, 0.8);
  content: '';
  pointer-events: none;
}
...
.sudoku-cell--selected {
  z-index: 1;
  box-shadow: inset 0 0 0 3px rgba(150, 81, 102, 0.96);
}
...
.sudoku-cell__notes span {
  display: grid;
  place-items: center;
  color: color-mix(in srgb, var(--sudoku-number-color) 78%, transparent);
  font-size: clamp(0.36rem, calc(var(--board-size) / 68), 0.72rem);
  font-weight: 800;
  line-height: 1;
}
</style>
